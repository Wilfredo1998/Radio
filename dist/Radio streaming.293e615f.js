...
async function loadImages() {
    try {
        ...
        console.error("\u274C Error cargando im\xe1genes:", error);
    }
}
async function updateCarousel(carruselId, images, folder) {
    ...
    if (!carouselWrapper) {
        console.error(`Error: No se encontr\xf3 el contenedor de ${carruselId}`);
        return;
    }
    carouselWrapper.innerHTML = "";
    carouselContainer.style.visibility = "hidden"; // Ocultar mientras carga
    if (images.length === 0) {
        console.error(`No se encontraron im\xe1genes en ${folder}.`);
        return;
    }
...
function startCarousel(carruselId) {
    ...
    let index = 0;
    if (items.length === 0) {
        console.error(`No hay im\xe1genes en ${carruselId}.`);
        return;
    }
...
document.addEventListener("DOMContentLoaded", function() {
    console.log("El DOM est\xe1 listo, pero a\xfan se est\xe1n cargando im\xe1genes...");
    loadImages();
});

//# sourceMappingURL=Radio streaming.293e615f.js.map
